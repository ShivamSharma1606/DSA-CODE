...
class TabularSolution
{
    int maximumPoints(vector<vector<int>> &arr, int n)
    {
        ...
        dp[0][3] = max(arr[0][0], max(arr[0][1], arr[0][2])); // it store the max value of that day [from all the given value]

        for (int day = 1; day < n; day++)
        {
            for (int last = 0; last <= 3; last++)
            /*
            we can also traverse for last <=2 but in that case we have to find the max from the last
            row of the dp array . last = 3 store the max value of that row hence we not need to compute 
            extra
            */
            {
